...
export async function generateGoogleStory(
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.warn('Google AI HTTP error for story:', response.status, errorText);
    throw new Error(`Google AI HTTP error: ${response.status} - ${errorText}`);
  }
  ...
  try {
    ...
    console.warn('Failed to parse Google AI story response as JSON:', textParts);
    throw new Error(`Google AI response was not valid JSON: ${err.message}`);
  }
