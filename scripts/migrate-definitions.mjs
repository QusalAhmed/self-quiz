...
async function migrateDatabase() {
  console.log('Running database migration for multi-definition support...');

  const sqlPath = path.resolve(__dirname, 'migrate-definitions.sql');
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST202') {
        ...
        console.warn(
          'Please manually copy the contents of "scripts/migrate-definitions.sql" and run them in your Supabase SQL Editor.'
        );
        process.exitCode = 0;
        return;
      }
