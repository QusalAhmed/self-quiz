  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST202') {
        ...
        console.warn(
          'Please manually copy the contents of "scripts/migrate-usage-frequency.sql" and run them in your Supabase SQL Editor.'
        );

        // Check if columns already exist by testing a select query
        const { error: testError } = await supabase
          .from('words')
          .select('usage_frequency, generator_ai_details')
          .limit(1);
