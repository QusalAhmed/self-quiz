...
async function migrateDatabase() {
  console.log('Running database migration for daily usage time support...');

  const sqlPath = path.resolve(__dirname, 'migrate-daily-usage.sql');
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST202') {
        ...
        console.warn(
          'Please manually copy the contents of "scripts/migrate-daily-usage.sql" and run them in your Supabase SQL Editor.'
        );
        process.exitCode = 0;
        return;
      }
